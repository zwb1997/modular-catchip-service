...
public class DetectService extends AbstractSchedulingService {
    ...
    protected void doSchedulingTask() {
        ...
        String sql = "SELECT COUNT(ip_num) FROM ip_pool_main";
        int ipCounts = jdbcTemplate.queryForObject(sql, new Object[] {},
                new BeanPropertyRowMapper<Integer>(Integer.class));
        int pos = 0;
        int step = ipCounts / ThreadPoolProvider.getCorePoolSize();
        while (pos < ipCounts) {
            ...
            try {
                THREAD_POOL_PROVIDER.submitTaskWork(() -> {
                    return null;
                });
            } catch (Exception e) {
